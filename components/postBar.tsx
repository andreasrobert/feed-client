...
export default function PostBar(props: { data: any }) {
  ...
  const getComments = async () => {
    const res = await fetch(
      `https://feed-database-postgres.herokuapp.com/comments/${props.data.id}`,
      {
        method: "GET",
      }
    );
  ...
  const checkPoint = async () => {
    const res = await fetch(
      "https://feed-database-postgres.herokuapp.com/checkPoint",
      {
        method: "POST",
        headers: {
  ...
  const addPoint = async () => {
    ...
    setNeutral(true);
    const res = await fetch(
      `https://feed-database-postgres.herokuapp.com/points`,
      {
        method: "POST",
        headers: {
          Accept: "application/json",
  ...
  const decreasePoint = async () => {
    ...
    setNeutral(false);
    const res = await fetch(
      `https://feed-database-postgres.herokuapp.com/point`,
      {
        method: "POST",
        headers: {
          Accept: "application/json",
        ...
        <Flex h="100%">
          ...
          >
            <Image
              ...
              filter={
                neutral
                  ? "invert(0) sepia(0) saturate(1) hue-rotate(0deg) brightness(1.2)"
                  : "invert(0.4) sepia(1) saturate(20) hue-rotate(190.8deg) brightness(0.78)"
              }
              mb="4px"
            ></Image>
            <style jsx global>{`
          ...
          <Flex flexDir="column" justifyContent="space-between">
            <Link
              href={`https://thefeed.netlify.app/post/${props.data.id}`}
              passHref
            >
            ...
            <Flex
              ...
              mt="3px"
            >
              <Text size="Body3">{`Posted by u/${props.data.username} ${dayjs(
